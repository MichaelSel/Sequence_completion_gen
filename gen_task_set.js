...
/**Set Settings here*/
async function gen_task_set(sub_id, prefix = "SVG", root = './task_sets', num_of_blocks = 10) {
    ...
    const practice = make_stimuli(sub_name, 3, 1, 0.3) /**Set Settings here*/
        .map((stimulus, Q_num) => {
            ...
            stimulus.probe_file = "P-" + (stimulus.Q_num) + "-000-probe.wav"
            stimulus.test_file = "P-" + (stimulus.Q_num) + "-001-test.wav"
            return stimulus
        })


    const experimental = [...Array(num_of_blocks).keys()].map(e => {
        return shuffle([
            ...make_stimuli(sub_name, 8, 2, 0.3, [0, 2, 4, 5, 7, 9, 11], "diatonic"), /**Set Settings here*/
            // ...make_stimuli(sub_name, 4, 2, 0.3, [0, 2, 4, 7, 9], "pentatonic") /**Set Settings here*/
        ])
    }).flat().map((stimulus, Q_num) => {
        stimulus.Q_num = Q_num + 1
        stimulus.probe_file = "Q-" + (stimulus.Q_num) + "-000-probe.wav"
        stimulus.test_file = "Q-" + (stimulus.Q_num) + "-001-test.wav"
        return stimulus
    })

    const blocks = [practice, ...split_stimuli_to_blocks(experimental,num_of_blocks)]
