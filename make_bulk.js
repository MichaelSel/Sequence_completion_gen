...
const make_txt = function (starting_id,num_of_sets,prefix="SeqInLab",root='./task_sets') {
    let subs = []
    ...
    subs = subs.join('\n')
    fs.writeFile(root + "/sequence_available.txt", subs, function(err) {
        if(err) {
...
const make_bulk = function (starting_id=0,num_of_sets=11,prefix="SeqMEG",root='./task_sets') {
    ...
    process_users(starting_id)
        ...
        fs.writeFile(root + "/sequence_available.txt", generated_sets, function(err) {
            if(err) {
                return console.log(err);
            }
        });
...
const make_list = function (list_of_sets,prefix="SeqC",root='./task_sets') {
    if (!fs.existsSync(root)){
        fs.mkdirSync(root);
    }
    let generated_sets = []
